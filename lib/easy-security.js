EasySecurity = (function () {
  ...
  var configuration = {
      ...
      methods: { someMethod: { type: 'throttle', ms: 2000 } },
  ...
  function getId(scope, rand) {
    ...
    if (scope && scope.connection) return scope.connection;
    else if (userId) return userId;
    else if (Meteor.isClient) return rand;
    else return null;
  }
  ...
  methods.rateLimit = {
    ...
    callFunctionsInQueue: function (id, funcScope, args) {
      var funcs = this.queues[id].functions,
        func = funcs.shift();

      if (func) {
        func.future.return(func.apply(funcScope, args));
      } else {
      ...
      return function () {
        ...
        if (id) {
          ...
          if (!methodScope.queues[id].timer) {
            ...
            methodScope.callFunctionsInQueue(id, funcScope, args);
        ...
        // TODO: make this configurable and better error message
        //throw new Meteor.Error('Should not get here!');
      ...
      return function () {
        ...
        if (id) {
          ...
          if (!methodScope.queues[id].previousCall || (now  - methodScope.queues[id].previousCall) >= ms) {
            ...
            funcScope.easySecurityData = collectData ? { data: _.clone(data) } : null;
  ...
  /*methods.debounce = {
    wrap: function debounce(func, ms, collectData) {
      var timeout = null,
        data = [];
      ...
      return function () {
        ...
        if (collectData) {
          data.push(Array.prototype.slice.call(args));
        }
        ...
        timeout = Meteor.setTimeout(function () {
          data = [];
          timeout = null;
          funcScope.easySecurityData = collectData ? { data: data } : null;
          return future.return(func.apply(funcScope, args));
        }, ms);
  ...
  return {
    ...
    'getSecuredFunction' : function (name, func) {
      ...
      return this.getMethod(conf.type).wrap(func, conf.ms);
    }
  };
})();
